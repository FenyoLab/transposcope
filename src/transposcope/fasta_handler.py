...
class FastaHandler:
    def __init__(self, line1_reference_path, reference_genome_folder_path, line1_start, line1_end):
        ...
        with open(line1_reference_path, 'rU') as handle:
            self._LINE1_REFERENCE_SEQ = next(SimpleFastaParser(handle))[1].upper()
    ...
    def reverse_complement(sequence):
        complement_sequence = ''.join([{'A': 'T', 'C': 'G',
                                        'G': 'C', 'T': 'A', 'N': 'N'}[x] for x in sequence.upper()])
