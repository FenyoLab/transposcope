        ...
        for read_key in keys_of_reads_in_target_region:
            ...
            fastq_read1.write(
                ...
                "\n{query_quality}\n".format(
                    ...
                    query_sequence=read_pair[0].query_sequence
                    if not read_pair[0].is_reverse
                    else self.reverse_complement(read_pair[0].query_sequence),
                    query_quality="".join(
                        chr(x + 33) for x in read_pair[0].query_qualities
                    ),
                )
            )
            fastq_read2.write(
                ...
                "\n{query_quality}\n".format(
                    ...
                    query_sequence=read_pair[1].query_sequence
                    if not read_pair[1].is_reverse
                    else self.reverse_complement(read_pair[1].query_sequence),
                    query_quality="".join(
                        chr(x + 33) for x in read_pair[1].query_qualities
                    ),
                )
            )
        fastq_read1.close()
