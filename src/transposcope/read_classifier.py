...
class ReadClassifier:
    ...
    def classify_insertion(self, insertion, bamfile):
        ...
        for (k, v) in read_dict.items():
            ...
            elif v[0].reference_start + 1 < j_end and \
                    v[0].reference_end > j_start:
            ...
            if v[1].is_unmapped:
                ...
                queries[3] = v[1]
            elif v[1].reference_start + 1 < j_end and \
                    v[1].reference_end > j_start:
        ...
        for (i, item) in enumerate(letter_g):
            if item['bp']['A'] + item['bp']['G'] + item['bp']['C'] + \
                    item['bp']['T'] + item['bp']['N'] != 0:
                letter_g[i]['c'] = max(
        ...
        with open(os.path.join(output_directory,
                               insertion.CHROMOSOME + '-' + str(clip_id) +
                               '(' + str(clip_flank) + ').json.gz.txt'),
                  'wb+') as outfile:
        ...
        if bins[0] != '' or bins[1] != '':
            if boolean_which_reads[0]:
                ...
                for block in blocks:
                    if complement:
                        b_start = block[0] + 1
                        b_end = block[1] + 1 if block[1] + 1 < mins[1] \
                            else mins[1]
                    else:
                        # TODO - make a variable for the ref length
                        b_start = block[0] + 1 if block[0] + 1 >= 1001 \
                            else 1001
                        b_end = block[1] + 1
            ...
            if boolean_which_reads[1]:
                ...
                for block in blocks:
                    if complement:
                        b_start = block[0] + 1 if block[0] + 1 >= mins[1] \
                            else mins[1]
                        b_end = block[1] + 1
                    else:
                        b_start = block[0] + 1
                        b_end = block[1] + 1 if block[1] + 1 <= 1001 \
                            else 1001
                    try:
                        for i in range(len(blocks) - 1):
                            for j in range(blocks[i][1], blocks[i + 1][0] + 1):
                        ...
                        for i in range(b_start, b_end):
                            ...
                            json_data['bins'][bins[1]][
                                'coverage'][i - mins[1]][
                                'pos'] = i + self._L1_HS_REF_SEQ_START if \
                                not complement else \
                ...
                for block in blocks:
                    ...
                    for i in range(b_start, b_end):
                        ...
                        json_data['bins'][bins[1]][
                            'coverage'][i - mins[1]][
                            'pos'] = i + self._L1_HS_REF_SEQ_START if \
                            not complement else self._L1_HS_REF_SEQ_END - (
                                i - mins[1])
    ...
    def colorize_g(seq, cigar, leng):
        ...
        ret_string = ' ' * leng + ret_string + ' ' * (
                200 - len(leng_string) - leng
        )
