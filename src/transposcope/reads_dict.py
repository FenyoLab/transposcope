class ReadsDict(dict):
    ...
    def insert(self, v):
        ...
        if len(found_read) == 2:
            ...
            raise ValueError("There should not be more than two reads with the same query name")
        elif len(found_read) == 0:
            raise ValueError("There should not be empty lists at this point")
        else:
            if found_read[0].is_read2 and v.is_read1 or found_read[0].is_read1 and v.is_read2:
