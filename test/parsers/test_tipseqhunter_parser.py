...
def repred_dataframe():
    return tipseqhunter_parser.load_repred('./examples/example.repred')
...
def test_repred_dataframe_validation(repred_dataframe):
    ...
    assert 'In row(s) [3 7] the clipping chromosome does not match the target chromosome.' in str(error.value)
