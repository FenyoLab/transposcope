...
Read = namedtuple('Read', ['query_name', 'is_read1', 'is_read2'])
...
class TestReadsDict(TestCase):
    ...
    def test_insert_read2_with_read1_present(self):
        reads_dict = ReadsDict({'a': [Read('a', False, True)]})
    ...
    def test_insert_read1_with_both_reads_present(self):
        reads_dict = ReadsDict({'a': [Read('a', False, True), Read('a', True, False)]})
        result = reads_dict.insert(Read('a', True, False))
    ...
    def test_insert_read1_with_empty_list(self):
        ...
        self.assertRaises(ValueError, reads_dict.insert, Read('a', True, False))
