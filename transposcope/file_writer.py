...
class FileWriter:
    ...
    def reverse_complement(sequence):
        ...
        complement_sequence = ''.join({'A': 'T', 'C': 'G',
                                       'G': 'C', 'T': 'A', 'N': 'N'}[x] for x in reverse_sequence)
        return complement_sequence

    def write_fastq(self, file_path, reads_dictionary, file_name, keys_of_reads_in_target_region):
        ...
        for read_key in keys_of_reads_in_target_region:
            ...
            fastq_read1.write(
                "@{query_name} 1:N:0:1\n{query_sequence}\n+\n{query_quality}\n".format(
                    ...
                    query_sequence=read_pair[0].query_sequence if not read_pair[
                        0].is_reverse else self.reverse_complement(
                        read_pair[0].query_sequence),
                    query_quality=''.join(chr(x + 33) for x in read_pair[0].query_qualities)
                ))
            fastq_read2.write(
                "@{query_name} 2:N:0:1\n{query_sequence}\n+\n{query_quality}\n".format(
                    query_name=read_pair[1].query_name,
                    query_sequence=read_pair[1].query_sequence if not read_pair[
                        1].is_reverse else self.reverse_complement(
                        read_pair[1].query_sequence),
                    query_quality=''.join(chr(x + 33) for x in read_pair[1].query_qualities)
                ))
        fastq_read1.close()
        fastq_read2.close()
